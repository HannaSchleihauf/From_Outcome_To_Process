...
## Inspect data ------------------------------------
xx <- ftable(better.job ~ culture + condition + age.group.sum, xdata)
xx
write.xlsx(xx, file = "./better.job.frequencies.xlsx")
table(xdata$better.job, xdata$culture, xdata$condition, xdata$age.group.sum, useNA = "ifany")
...
xx <- xdata[is.na(xdata$better.job), ]
table(xx$condition, xx$culture)

# interrater reli
reli.data <- subset(xdata, xdata$better.job.reli != "NA")
ratings <- reli.data %>% select(better.job, better.job.reli)
library(irr)
agree(ratings)
...
xx.fe.re <- fe.re.tab(
  fe.model = "better.job ~ condition*age.group.sum*culture",
  re = "(1|sub.id)", other.vars = c("age", "age.group"), data = xdata
)
xx.fe.re$summary
t.data <- xx.fe.re$data
str(t.data) # in t.data the 11 observations in which children did not maka a choice are excluded

# t.data = subset(t.data, t.data$age.group.sum != "8.9")
...
full <- glmer(better.job ~ condition * age.group.sum * culture +
...
t.data$resp <- as.numeric(t.data$better.job)
...
to.change.1 <- (1:nrow(t.data))[(t.data$condition == "outcome.only" & t.data$age.group.sum == "8.9" & t.data$culture == "china")]
to.change.2 <- (1:nrow(t.data))[(t.data$condition == "process.only" & t.data$age.group.sum == "8.9" & t.data$culture == "china")]
to.change.3 <- (1:nrow(t.data))[(t.data$condition == "process.only" & t.data$age.group.sum == "8.9" & t.data$culture == "usa")]
to.change.4 <- (1:nrow(t.data))[(t.data$condition == "process.only" & t.data$age.group.sum == "adult" & t.data$culture == "usa")]
...
# save.image("./supplementary.analysis.better.job.1000.RData")
load("./supplementary.analysis.better.job.1000.RData")

## Evaluation of the results -----------------------------------------------
str(all.res)
# how many models did converge
# full
sum(all.res$full.warnings == "no") # 270 of the models converged
# null
sum(all.res$null.warnings == "no") # 270 of the models converged
# red
...
zz <- round(zz, 3)
better.job.boot.full <- cbind(xx[, 1], yy[, 2], zz[, 3])

write.table(better.job.boot.full, "analysis.better.job.boot.full.txt", sep = "\t", row.names = TRUE, na = "NA", dec = ".")
...
save.image("./R_images/PRR.analysis.better.job.RData")
