# Project: PRR
# Analysis: Help-Seeking Variable only Outcome vs. Process Condition
# Author: Hanna Schleihauf
# Date: September 23 2021

## Analysis with the only the data from the outcome vs process condition: Participants of all age groups --------
...
## Inspect data ------------------------------------
xx <- ftable(help.seeking ~ culture + age.group.sum, ovpdata)
xx
# we do not have a complete separation problem

# how many trials are excluded because children did not make a choice
xx <- ovpdata[is.na(ovpdata$help.seeking), ] # --> none

## Prepare data for model fitting------------------------------------
xx.fe.re <- fe.re.tab(
  fe.model = "help.seeking ~ age.group.sum*culture",
  re = "(1|sub.id)", other.vars = c("age", "age.group", "trial.nr"), data = xdata
)
xx.fe.re$summary
...
full <- glmer(help.seeking ~ age.group.sum * culture +
  (1 | sub.id),
...
xx <- glmer(help.seeking ~ age.group.sum + culture +
  (1 | sub.id),
data = t.data, control = contr, family = binomial(link = "logit")
)
library(car)
...
m.stab.b <- glmm.model.stab(model.res = full, contr = contr, use = c("sub.id"))
m.stab.b$detailed$warnings
xx <- as.data.frame(round(m.stab.b$summary[, -1], 3))
dev.off()
m.stab.plot(round(m.stab.b$summary[, -1], 3))
write.table(xx, "analysis.help.seeking.outcome.vs.process.m.stab.txt", quote = FALSE, sep = "\t")
xx

# null model
null <- glmer(help.seeking ~ 1 +
  (1 | sub.id),
data = t.data, control = contr, family = binomial(link = "logit")
)
...
# main effects model
main <- glmer(help.seeking ~ age.group.sum + culture +
  (1 | sub.id),
data = t.data, control = contr, family = binomial(link = "logit")
)
...
tests <- drop1p(model.res = main, para = F, data = NULL, contr = contr, n.cores = c("all-1", "all"), to.del = NULL)
round(tests$drop1.res, 3)
...
save.image("./R_images/analysis.better.help.seeking.vs.process.RData")
